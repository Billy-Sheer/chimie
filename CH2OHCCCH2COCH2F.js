...
function verifierHydrogenes(symboles, voisins) {
  ...
  for (var i = 0; i < symboles.length; i++) {
    ...
    if (symboles[i] === "H") {
      ...
      var vH = voisins[i + 1];
      if (vH.length !== 1 || symboles[vH[0].atom - 1] !== "O") {
...
function verifierValence(symboles, voisins) {
  ...
  for (var i = 0; i < symboles.length; i++) {
    ...
    if (s === "F" && nb !== 1) return "Un halogène fait toujours un seul lien.";
  }
...
function verifierStructure(symboles, voisins, liaisons) {
  ...
  for (var i = 0; i < symboles.length; i++) {
    if (symboles[i] === "O") {
      for (var v of voisins[i + 1]) {
        if (symboles[v.atom - 1] === "H") alcool = true;
      }
    }
  }
  if (!alcool) return "La liaison O–H est absente.";
  ...
  if (nCC3 !== 1) return "Il doit y avoir une seule liaison triple entre deux carbones.";
