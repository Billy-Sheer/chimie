...
function verifierValence(symboles, voisins) {
  ...
  for (var i = 0; i < symboles.length; i++) {
    ...
    if ((s === "F" || s === "Br") && nb !== 1) {
      return "Un halogène doit avoir un seul lien.";
    }
  }
...
function verifierStructure(symboles, voisins, liaisons) {
  ...
  // 1) Une seule liaison triple
  if (compterLiaisons(liaisons, 3) !== 1) {
    return "Il doit y avoir une seule liaison triple C≡C.";
  }

  // 2) Aucune liaison double
  if (compterLiaisons(liaisons, 2) !== 0) {
    return "Il ne doit pas y avoir de liaison double dans cette molécule.";
  }

  // 3) CF2 : un carbone avec exactement 2 F
  let cf2 = false;
  ...
  if (!cf2) {
    return "Les deux fluor doivent être sur le même carbone.";
  }
...
function verifier() {
  ...
  ketcher.getMolfile().then(function (mol) {
    ...
    if (compter(symboles, "Br") !== 1) {
      feedback.innerHTML = "Le nombre d'atomes de brome est incorrect.";
      return;
    }
