  ...
  for (var i = 0; i < symboles.length; i++) {
    if (symboles[i] === "C") {
      ...
      for (var v of voisins[i + 1]) {
        if (symboles[v.atom - 1] === "F") nF++;
      }
      if (nF === 2) cf2 = true;
    }
  }
  if (!cf2) return "Les deux fluor doivent être sur le même carbone.";

  // Au moins un embranchement (carbone avec 3 voisins C)
  let embranchement = false;
  for (var i = 0; i < symboles.length; i++) {
    if (symboles[i] === "C") {
      let nC = 0;
      for (var v of voisins[i + 1]) {
      ...
      if (nC >= 3) embranchement = true;
    }
  }
  if (!embranchement) {
    return "Un embranchement est attendu dans cette molécule.";
  }
